...
function useSpring(initialValue, settings = {
  stiffness: 0.066,
  damping: 0.25
}) {
  const [value, setValue] = useState(initialValue);
  const [targetValue, setTargetValue] = useState(initialValue);
  const [velocity, setVelocity] = useState(typeof initialValue === 'object' ? Object.keys(initialValue).reduce((acc, key) => ({
    ...acc,
    [key]: 0
  }), {}) : 0);
  ...
  const set = (newTargetValue, options = {}) => {
    if (options.hard) {
      ...
      setVelocity(typeof newTargetValue === 'object' ? Object.keys(newTargetValue).reduce((acc, key) => ({
        ...acc,
        [key]: 0
      }), {}) : 0);
    } else {
...
/**
 * ThreeDCard - An interactive 3D card component with holographic effects
 * 
 * @param {Object} props
 * @param {string} props.img - Front image URL (required)
...
export default function ThreeDCard({
  ...
  const [springScale, setSpringScale] = useSpring(1, {
    stiffness: 0.033,
    damping: 0.45
  });
  ...
  const handleClick = () => {
    ...
    if (!active) {
      ...
      // Deactivate: zoom out without flipping
      setActive(false);
      ...
      // Use faster spring settings for zoom out
      setSpringScale(1, {
        stiffness: 0.132,
        damping: 0.45
      });
      ...
      children: /*#__PURE__*/_jsx("div", {
        ...
        children: /*#__PURE__*/_jsx("div", {
          style: {
            ...
            boxShadow: (active || interact) && !disableHoverCardGlow ? '0 0 3px -1px white, 0 0 3px 1px hsl(47, 100%, 78%), 0 0 12px 2px hsl(175, 100%, 90%), 0px 10px 20px -5px black, 0 0 40px -30px hsl(175, 100%, 90%), 0 0 50px -20px hsl(175, 100%, 90%)' : '0 0 3px -1px transparent, 0 0 2px 1px transparent, 0 0 5px 0px transparent, 0px 10px 20px -5px black, 0 2px 15px -5px black, 0 0 20px 0px transparent'
          },
