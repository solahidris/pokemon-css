...
function useSpring(initialValue: any, settings = { stiffness: 0.066, damping: 0.25 }) {
  const [value, setValue] = useState(initialValue);
  const [targetValue, setTargetValue] = useState(initialValue);
  const [velocity, setVelocity] = useState(
    typeof initialValue === 'object' 
      ? Object.keys(initialValue).reduce((acc: any, key) => ({ ...acc, [key]: 0 }), {})
      : 0
  );
  ...
  const set = (newTargetValue: any, options: any = {}) => {
    if (options.hard) {
      setValue(newTargetValue);
      setTargetValue(newTargetValue);
      setVelocity(
        typeof newTargetValue === 'object'
          ? Object.keys(newTargetValue).reduce((acc: any, key) => ({ ...acc, [key]: 0 }), {})
          : 0
      );
    } else {
      setTargetValue(newTargetValue);
...
interface ThreeDCardProps {
  ...
  disableHoverCardGlow?: boolean;
  disableFlare?: boolean;
...
export function ThreeDCard({ 
  ...
  const handleMouseMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (!interact || !cardRef.current) return;
  ...
  const handleClick = () => {
    if (disableClickEnlarge || !cardRef.current) return;
